 ...
 * @return void
 */
function bluetone_register_block_patterns()
{
    $block_pattern_categories = array(
        'bluetone' => array('label' => esc_html__('bluetone', 'bluetone')),
        'pages'  => array('label' => esc_html__('Pages', 'bluetone')),
    );
     ...
     * }
     */
    $block_pattern_categories = apply_filters('bluetone_block_pattern_categories', $block_pattern_categories);
        ...
        if (is_file($file_path) && pathinfo($file_path, PATHINFO_EXTENSION) == 'php') {
            ...
            if ($file !== 'pricing-table-2.php') {
                ...
                if (!empty($pattern_content)) {
                    ...
                    $pattern_name = 'bluetone-pattern-' . sanitize_title($pattern_title); // Generate a unique name for the pattern
...
add_action('init', 'bluetone_register_block_patterns', 9);
